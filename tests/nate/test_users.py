...
base_user = auth.User(
    ...
    role=auth.Role(id=0, name='Chapter Master'),
)

base_token = auth.Token(
    id=uuid4(),
    token='token',
...
class TestUsers(BaseTestComponent):
    ...
    def setUp(self):
        super().setUp()
        self.users = Users(http_client=self.http_client,
    ...
    def test_get_all(self):
        ...
        self.assertEqual(users, [auth.User.from_transport_model(u) for u in t_models.root])
    ...
    def test_get_tokens(self):
        ...
        self.assertEqual(tokens, [auth.Token.from_transport_model(t) for t in t_models.root])
